component extends="tests.resources.ModuleIntegrationSpec" appMapping="/app" {
    ...
    function run() {
        describe( "preUpdate spec", function() {
            it( "announces a quickPreUpdate interception point", function() {
                ...
                expect( request.quickPreUpdateCalled[ 1 ].entity.getDownloadUrl() ).toBe( "https://open.spotify.com/track/0GHGd3jYqChGNxzjqgRZSv" );
                structDelete( request, "quickPreUpdateCalled" );
            } );
    ...
    function quickPreUpdate( event, interceptData, buffer, rc, prc ) {
