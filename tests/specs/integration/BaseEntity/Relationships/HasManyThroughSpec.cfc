	...
	function run() {
		describe( "Has Many Through Spec", function() {
			...
			it( "can get the related entities through another entity", function() {
				...
				expect( countryA.getPosts()[ 1 ].getPost_Pk() ).toBe( 523526 );
				expect( countryA.getPosts()[ 1 ].getBody() ).toBe( "My second awesome post body" );
				expect( countryA.getPosts()[ 2 ].getPost_Pk() ).toBe( 1245 );
				expect( countryA.getPosts()[ 2 ].getBody() ).toBe( "My awesome post body" );
			...
			it( "can get the related entities through another entity including a constrained relationship", function() {
				var countryA = getInstance( "Country@something" ).find( "02B84D66-0AA0-F7FB-1F71AFC954843861" );
				expect( arrayLen( countryA.getPublishedPosts() ) ).toBe( 1 );
				expect( countryA.getPublishedPosts()[ 1 ].getPost_Pk() ).toBe( 1245 );
