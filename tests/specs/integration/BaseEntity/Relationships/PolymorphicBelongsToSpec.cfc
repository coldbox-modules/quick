    ...
    function run() {
        describe( "Polymorphic Belongs To Spec", function() {
            it( "can get the related polymorphic entity", function() {
                ...
                expect( commentA.getCommentable() ).toBeInstanceOf(
                    "app.models.Post"
                );
                expect( commentA.getCommentable().getBody() ).toBe(
                    "My awesome post body"
                );

                var commentB = getInstance( "Comment" ).find( 2 );
                expect( commentB.getCommentable() ).toBeInstanceOf(
                    "app.models.Post"
                );
                ...
                expect( commentC.getCommentable() ).toBeInstanceOf(
                    "app.models.Video"
                );
                expect( commentC.getCommentable().getTitle() ).toBe(
                    "Cello Wars"
                );
            } );
        } );
